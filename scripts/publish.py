#!/usr/bin/env python3
"""Build the spectask-mcp package with uv and publish wheels/sdists to PyPI.
...
Stale ``*.whl`` and ``*.tar.gz`` files under ``dist/`` next to ``pyproject.toml`` are removed
before each ``uv build`` so ``uv publish`` uploads only the fresh artifacts.

Requires uv on PATH: https://docs.astral.sh/uv/

Authentication (CLI overrides environment):
  SPECTASK_MCP_PYPI_TOKEN  Environment variable (preferred for interactive use).
...
Usage:
  SPECTASK_MCP_PYPI_TOKEN=... python scripts/publish.py
  python scripts/publish.py --token pypi-...
  python scripts/publish.py --pyproject path/to/pyproject.toml
"""
...
def bump_pyproject_version(pyproject: Path) -> str:
    ...
    if idx is None:
        print(
            f"{_MSG_PREFIX} no version key in [project] root of pyproject.toml.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    line = lines[idx]
    ...
    if not m:
        print(
            f"{_MSG_PREFIX} expected version = \"...\" in pyproject.toml line {idx + 1}.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    try:
        ...
        print(f"{_MSG_PREFIX} {e}", file=sys.stderr)
...
def main() -> None:
    ...
    if not token:
        print(
            f"{_MSG_PREFIX} set {_TOKEN_ENV} or pass --token (see script docstring).",
            file=sys.stderr,
        )
        raise SystemExit(1)
    ...
    if not pyproject.is_file():
        print(
            f"{_MSG_PREFIX} pyproject.toml not found at {pyproject}",
            file=sys.stderr,
        )
        raise SystemExit(1)
    ...
    remove_dist_build_artifacts(pkg_root / "dist")
