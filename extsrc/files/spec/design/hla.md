...
All Jira HTTPS calls share `httpx.Client` from `build_httpx_client` in `spectask_mcp/jira/http_common.py`, with redirect following enabled (`follow_redirects=True`) so 3xx responses from proxies or canonical URL rules are resolved before handling status and payloads.
