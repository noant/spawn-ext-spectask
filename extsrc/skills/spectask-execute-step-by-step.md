...
3. Launch **one** sub-agent for that step — do NOT implement inline. Prefer `Suggested model` (Task `model` when supported; else prompt + nearest slug). Include **`R13-model-line`**.
4. Rename subtask to `_DONE_{name}`; **you (coordinator)** set `Status: Done` and `Used model` per **`R13-model-line`** (tool `model` if passed; else `My model:`). Sub-agent must not write `Used model`. `Suggested model` unchanged.
