...
13. **`R13-model-line`** — каждый запрос подагента должен включать эту строку дословно:
    ...
    Запись `[model-name]` для статуса, произведённого подагентом: прочитайте последнюю строку его ответа, извлеките имя модели и впишите в скобки. Для статуса, произведённого текущим агентом (Steps 1, 7), впишите собственное имя модели.
...
→ установить [V] "Design documents updated" `[model-name]` (запись модели по `R13-model-line`): `- [V] Design documents updated [model-name]`
...
5. Выполните **`spawn rules refresh`**.
