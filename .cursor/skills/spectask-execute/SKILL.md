...
Every sub-agent launch: **Subagent run protocol** (`R16-ambient`) — resolve Ambient context first (clarify if missing; `Ambient context: none` means set); put the Ambient block first in the prompt; pass it unchanged to children.

Step 4: prefer each subtask `Suggested model` (Task `model` when supported; else prompt + nearest slug). Include **Subagent run protocol**, **`R13-model-line`**, **`R14-changed-files`**, and Step 4 **Coder protocol** (worker sets `Status: Done`; does not rename). After each subtask, mark done per **`R15-done-marking`** (rename + `Used model`). Aggregate changed-file lists and forward them per **`R14-changed-files`**.

Step 5: **Subagent run protocol**, then **`R13-model-line`**; fill `- [V] Code self-review [model-name]` per **`R13-model-line`**; forward any further changed files per **`R14-changed-files`**.
